!(function (t, e) {
  "object" == typeof exports && "object" == typeof module
    ? (module.exports = e(
        require("d3-time"),
        require("d3-shape"),
        require("d3-selection"),
        require("d3-format"),
        require("d3-array"),
        require("d3-scale"),
        require("d3-ease"),
        require("d3-transition"),
        require("d3-dispatch"),
        require("d3-interpolate"),
      ))
    : "function" == typeof define && define.amd
    ? define(
        [
          "d3-time",
          "d3-shape",
          "d3-selection",
          "d3-format",
          "d3-array",
          "d3-scale",
          "d3-ease",
          "d3-transition",
          "d3-dispatch",
          "d3-interpolate",
        ],
        e,
      )
    : "object" == typeof exports
    ? (exports.donut = e(
        require("d3-time"),
        require("d3-shape"),
        require("d3-selection"),
        require("d3-format"),
        require("d3-array"),
        require("d3-scale"),
        require("d3-ease"),
        require("d3-transition"),
        require("d3-dispatch"),
        require("d3-interpolate"),
      ))
    : ((t.britecharts = t.britecharts || {}),
      (t.britecharts.donut = e(
        t["d3-time"],
        t["d3-shape"],
        t["d3-selection"],
        t["d3-format"],
        t["d3-array"],
        t["d3-scale"],
        t["d3-ease"],
        t["d3-transition"],
        t["d3-dispatch"],
        t["d3-interpolate"],
      )));
})(window, function (t, e, n, r, o, a, i, c, u, l) {
  return (function (t) {
    var e = {};
    function n(r) {
      if (e[r]) return e[r].exports;
      var o = (e[r] = { i: r, l: !1, exports: {} });
      return t[r].call(o.exports, o, o.exports, n), (o.l = !0), o.exports;
    }
    return (
      (n.m = t),
      (n.c = e),
      (n.d = function (t, e, r) {
        n.o(t, e) || Object.defineProperty(t, e, { enumerable: !0, get: r });
      }),
      (n.r = function (t) {
        "undefined" != typeof Symbol &&
          Symbol.toStringTag &&
          Object.defineProperty(t, Symbol.toStringTag, {
            value: "Module",
          }),
          Object.defineProperty(t, "__esModule", { value: !0 });
      }),
      (n.t = function (t, e) {
        if ((1 & e && (t = n(t)), 8 & e)) return t;
        if (4 & e && "object" == typeof t && t && t.__esModule) return t;
        var r = Object.create(null);
        if (
          (n.r(r),
          Object.defineProperty(r, "default", {
            enumerable: !0,
            value: t,
          }),
          2 & e && "string" != typeof t)
        )
          for (var o in t)
            n.d(
              r,
              o,
              function (e) {
                return t[e];
              }.bind(null, o),
            );
        return r;
      }),
      (n.n = function (t) {
        var e =
          t && t.__esModule
            ? function () {
                return t.default;
              }
            : function () {
                return t;
              };
        return n.d(e, "a", e), e;
      }),
      (n.o = function (t, e) {
        return Object.prototype.hasOwnProperty.call(t, e);
      }),
      (n.p = ""),
      n((n.s = 51))
    );
  })([
    function (e, n) {
      e.exports = t;
    },
    function (t, n) {
      t.exports = e;
    },
    function (t, e) {
      t.exports = n;
    },
    function (t, e) {
      t.exports = r;
    },
    function (t, e) {
      t.exports = o;
    },
    function (t, e, n) {
      "use strict";
      n.d(e, "a", function () {
        return a;
      }),
        n.d(e, "f", function () {
          return i;
        }),
        n.d(e, "d", function () {
          return c;
        }),
        n.d(e, "e", function () {
          return u;
        }),
        n.d(e, "b", function () {
          return l;
        }),
        n.d(e, "c", function () {
          return s;
        }),
        n.d(e, "g", function () {
          return f;
        });
      var r = n(1),
        o = n(0),
        a = {
          MINUTE_HOUR: "minute-hour",
          HOUR_DAY: "hour-daymonth",
          DAY_MONTH: "day-month",
          MONTH_YEAR: "month-year",
          CUSTOM: "custom",
        },
        i = {
          ONE_AND_A_HALF_YEARS: 47304e6,
          ONE_YEAR: 31536000365,
          ONE_DAY: 86400001,
        },
        c = [
          "00h",
          "01h",
          "02h",
          "03h",
          "04h",
          "05h",
          "06h",
          "07h",
          "08h",
          "09h",
          "10h",
          "11h",
          "12h",
          "13h",
          "14h",
          "15h",
          "16h",
          "17h",
          "18h",
          "19h",
          "20h",
          "21h",
          "22h",
          "23h",
        ],
        u = { duration: 1200 },
        l = {
          linear: r.curveLinear,
          basis: r.curveBasis,
          cardinal: r.curveCardinal,
          catmullRom: r.curveCatmullRom,
          monotoneX: r.curveMonotoneX,
          monotoneY: r.curveMonotoneY,
          natural: r.curveNatural,
          step: r.curveStep,
          stepAfter: r.curveStepAfter,
          stepBefore: r.curveStepBefore,
        },
        s = [{ quantity: 1, percentage: 100 }],
        f = {
          timeMillisecond: o.timeMillisecond,
          utcMillisecond: o.utcMillisecond,
          timeSecond: o.timeSecond,
          utcSecond: o.utcSecond,
          timeMinute: o.timeMinute,
          utcMinute: o.utcMinute,
          timeHour: o.timeHour,
          utcHour: o.utcHour,
          timeDay: o.timeDay,
          utcDay: o.utcDay,
          timeWeek: o.timeWeek,
          utcWeek: o.utcWeek,
          timeSunday: o.timeSunday,
          utcSunday: o.utcSunday,
          timeMonday: o.timeMonday,
          utcMonday: o.utcMonday,
          timeTuesday: o.timeTuesday,
          utcTuesday: o.utcTuesday,
          timeWednesday: o.timeWednesday,
          utcWednesday: o.utcWednesday,
          timeThursday: o.timeThursday,
          utcThursday: o.utcThursday,
          timeFriday: o.timeFriday,
          utcFriday: o.utcFriday,
          timeSaturday: o.timeSaturday,
          utcSaturday: o.utcSaturday,
          timeMonth: o.timeMonth,
          utcMonth: o.utcMonth,
          timeYear: o.timeYear,
          utcYear: o.utcYear,
        };
    },
    function (t, e) {
      t.exports = a;
    },
    function (t, e, n) {
      "use strict";
      n.r(e),
        n.d(e, "colorGradients", function () {
          return r;
        }),
        n.d(e, "colorGradientsHuman", function () {
          return o;
        }),
        n.d(e, "britecharts", function () {
          return a;
        }),
        n.d(e, "grey", function () {
          return i;
        }),
        n.d(e, "orange", function () {
          return c;
        }),
        n.d(e, "blueGreen", function () {
          return u;
        }),
        n.d(e, "teal", function () {
          return l;
        }),
        n.d(e, "green", function () {
          return s;
        }),
        n.d(e, "yellow", function () {
          return f;
        }),
        n.d(e, "pink", function () {
          return d;
        }),
        n.d(e, "purple", function () {
          return p;
        }),
        n.d(e, "red", function () {
          return h;
        }),
        n.d(e, "colorSchemas", function () {
          return g;
        }),
        n.d(e, "colorSchemasHuman", function () {
          return m;
        }),
        n.d(e, "aloeGreen", function () {
          return y;
        }),
        n.d(e, "greenColor", function () {
          return v;
        }),
        n.d(e, "blueColor", function () {
          return b;
        }),
        n.d(e, "yellowColor", function () {
          return C;
        }),
        n.d(e, "orangeColor", function () {
          return x;
        });
      var r = {
          greenBlue: ["#39C7EA", "#4CDCBA"],
          orangePink: ["#FBC670", "#F766B8"],
          bluePurple: ["#3DC3C9", "#824a9e"],
        },
        o = {
          greenBlue: "Green to Blue",
          orangePink: "Orange to Pink",
          bluePurple: "Blue to Purple",
        },
        a = ["#6aedc7", "#39c2c9", "#ffce00", "#ffa71a", "#f866b9", "#998ce3"],
        i = [
          "#F8F8FA",
          "#EFF2F5",
          "#D2D6DF",
          "#C3C6CF",
          "#ADB0B6",
          "#666A73",
          "#45494E",
          "#363A43",
          "#282C35",
        ],
        c = [
          "#fcc870",
          "#ffa71a",
          "#fb8825",
          "#f6682f",
          "#db5a2c",
          "#bf4c28",
          "#a43b1c",
          "#892a10",
          "#f9e9c5",
        ],
        u = [
          "#ccf7f6",
          "#70e4e0",
          "#00d8d2",
          "#00acaf",
          "#007f8c",
          "#005e66",
          "#003c3f",
          "#002d2f",
          "#0d2223",
        ],
        l = [
          "#ccfffe",
          "#94f7f4",
          "#00fff8",
          "#1de1e1",
          "#39c2c9",
          "#2e9a9d",
          "#227270",
          "#1a5957",
          "#133f3e",
        ],
        s = [
          "#edfff7",
          "#d7ffef",
          "#c0ffe7",
          "#95f5d7",
          "#6aedc7",
          "#59c3a3",
          "#479980",
          "#34816a",
          "#206953",
        ],
        f = [
          "#f9f2b3",
          "#fbe986",
          "#fce05a",
          "#fed72d",
          "#ffce00",
          "#fcc11c",
          "#f9b438",
          "#eda629",
          "#e09819",
        ],
        d = [
          "#fdd1ea",
          "#fb9cd2",
          "#f866b9",
          "#fc40b6",
          "#ff1ab3",
          "#e3239d",
          "#c62c86",
          "#a62073",
          "#85135f",
        ],
        p = [
          "#ddd6fc",
          "#bbb1f0",
          "#998ce3",
          "#8e6bc1",
          "#824a9e",
          "#77337f",
          "#6b1c60",
          "#591650",
          "#470f3f",
        ],
        h = [
          "#ffd8d4",
          "#ffb5b0",
          "#ff938c",
          "#ff766c",
          "#ff584c",
          "#f04b42",
          "#e03d38",
          "#be2e29",
          "#9c1e19",
        ],
        g = {
          britecharts: a,
          grey: i,
          orange: c,
          blueGreen: u,
          teal: l,
          green: s,
          yellow: f,
          pink: d,
          purple: p,
          red: h,
        },
        m = {
          britecharts: "Britecharts Default",
          grey: "Britecharts Grey",
          orange: "Orange",
          blueGreen: "Blue",
          teal: "Light Blue",
          green: "Green",
          yellow: "Yellow",
          pink: "Pink",
          purple: "Purple",
          red: "Red",
        },
        y = ["#7bdcc0"],
        v = ["#6aedc7"],
        b = ["#39c2c9"],
        C = ["#ffce00"],
        x = ["#ffa71a"],
        O = {
          aloeGreen: y,
          greenColor: v,
          blueColor: b,
          yellowColor: C,
          orangeColor: x,
          pinkColor: ["#f866b9"],
          purpleColor: ["#998ce3"],
        };
      e.default = {
        colorSchemas: g,
        colorSchemasHuman: m,
        colorGradients: r,
        colorGradientsHuman: o,
        singleColors: O,
        singleColorsHuman: {
          aloeGreen: "Aloe Green",
          greenColor: "Green",
          blueColor: "Blue",
          yellowColor: "Yellow",
          orangeColor: "Orange",
          pinkColor: "Pink",
          purpleColor: "Purple",
        },
      };
    },
    function (t, e) {
      (t.exports = function (t, e, n) {
        return (
          e in t
            ? Object.defineProperty(t, e, {
                value: n,
                enumerable: !0,
                configurable: !0,
                writable: !0,
              })
            : (t[e] = n),
          t
        );
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    function (t, e, n) {
      var r = n(25),
        o = n(26),
        a = n(15),
        i = n(27);
      (t.exports = function (t) {
        return r(t) || o(t) || a(t) || i();
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    function (t, e) {
      function n(e) {
        return (
          (t.exports = n =
            "function" == typeof Symbol && "symbol" == typeof Symbol.iterator
              ? function (t) {
                  return typeof t;
                }
              : function (t) {
                  return t &&
                    "function" == typeof Symbol &&
                    t.constructor === Symbol &&
                    t !== Symbol.prototype
                    ? "symbol"
                    : typeof t;
                }),
          (t.exports.__esModule = !0),
          (t.exports.default = t.exports),
          n(e)
        );
      }
      (t.exports = n),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    ,
    function (t, e) {
      (t.exports = function (t, e) {
        (null == e || e > t.length) && (e = t.length);
        for (var n = 0, r = new Array(e); n < e; n++) r[n] = t[n];
        return r;
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    ,
    ,
    function (t, e, n) {
      var r = n(12);
      (t.exports = function (t, e) {
        if (t) {
          if ("string" == typeof t) return r(t, e);
          var n = Object.prototype.toString.call(t).slice(8, -1);
          return (
            "Object" === n && t.constructor && (n = t.constructor.name),
            "Map" === n || "Set" === n
              ? Array.from(t)
              : "Arguments" === n ||
                /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)
              ? r(t, e)
              : void 0
          );
        }
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    ,
    function (t, e) {
      t.exports = i;
    },
    function (t, e, n) {
      "use strict";
      n.d(e, "a", function () {
        return h;
      });
      var r = n(10),
        o = n.n(r),
        a = n(19),
        i = n.n(a),
        c = n(7),
        u = n(20),
        l = n.n(u),
        s = "undefined" != typeof window,
        f = navigator.msSaveOrOpenBlob,
        d = s && window.btoa;
      d || (d = i.a.encode);
      var p = {
        styleClass: "britechartStyle",
        defaultFilename: "britechart.png",
        chartBackground: "white",
        imageSourceBase: "data:image/svg+xml;base64,",
        titleFontSize: "15px",
        titleFontFamily: "‘Helvetica Neue’, Helvetica, Arial, sans-serif",
        titleTopOffset: 15,
        get styleBackgroundString() {
          return "<style>svg{background:".concat(
            this.chartBackground,
            ";}</style>",
          );
        },
      };
      function h(t, e, n) {
        var r = this;
        return f
          ? (console.error(
              "Sorry, this feature is not available for IE. If you require this to work, check this issue https://github.com/eventbrite/britecharts/pull/652",
            ),
            !1)
          : C(g.call(this, t, n))
              .then(function (t) {
                return {
                  canvas: m(r.width(), r.height()),
                  img: t,
                };
              })
              .then(function (t) {
                var n = t.canvas,
                  r = t.img;
                return b.call(r, n, e);
              });
      }
      function g(t, e) {
        if (t) {
          t.attr("version", 1.1).attr("xmlns", "http://www.w3.org/2000/svg");
          var n = l.a.initializeSerializer()(t.node());
          return (
            (n = (function (t) {
              if (navigator.userAgent.search("FireFox") > -1)
                return t.replace(
                  /url.*&quot;\)/,
                  'url(&quot;linearGradient[id*="-gradient-"]&quot;);',
                );
              return t;
            })(n)),
            (n = (function (t) {
              return t.replace(">", ">".concat(p.styleBackgroundString));
            })((n = x.call(this, n, e, parseInt(t.attr("width"), 10)))))
          );
        }
      }
      function m(t, e) {
        var n = document.createElement("canvas");
        return (n.height = e), (n.width = t), n;
      }
      function y(t, e) {
        var n = new Image();
        if (e) {
          if ("function" != typeof e)
            throw new Error(
              "The callback provided should be a function, we got a ".concat(
                o()(e),
                " instead.",
              ),
            );
          e(n);
        }
        return (
          (n.src = "".concat(p.imageSourceBase).concat(
            d(
              encodeURIComponent(t).replace(/%([0-9A-F]{2})/g, function (t, e) {
                return String.fromCharCode("0x" + e);
              }),
            ),
          )),
          n
        );
      }
      function v(t, e) {
        return e.getContext("2d").drawImage(t, 0, 0), e;
      }
      function b(t, e) {
        !(function (t) {
          var e =
              arguments.length > 1 && void 0 !== arguments[1]
                ? arguments[1]
                : p.defaultFilename,
            n =
              arguments.length > 2 && void 0 !== arguments[2]
                ? arguments[2]
                : "image/png",
            r = t.toDataURL(n),
            o = document.createElement("a");
          (o.href = r),
            (o.download = e),
            document.body.appendChild(o),
            o.click(),
            document.body.removeChild(o);
        })(v(this, t), e);
      }
      function C(t) {
        return new Promise(function (e, n) {
          y(t, function (t) {
            t.addEventListener("load", function () {
              return e(t);
            }),
              t.addEventListener("error", function (t) {
                return n(t);
              });
          });
        });
      }
      function x(t, e, n) {
        if (!e || !n) return t;
        var r = c.colorSchemas.grey;
        return (t = t.replace(
          /<g/,
          '<text x="'
            .concat(this.margin().left, '" y="')
            .concat(p.titleTopOffset, '" font-family="')
            .concat(p.titleFontFamily, '" font-size="')
            .concat(p.titleFontSize, '" fill="')
            .concat(r[6], '"> ')
            .concat(e, " </text><g "),
        ));
      }
    },
    function (t, e, n) {
      (function (t, r) {
        var o;
        /*! http://mths.be/base64 v0.1.0 by @mathias | MIT license */ !(function (
          a,
        ) {
          var i = e,
            c = (t && t.exports, "object" == typeof r && r);
          c.global !== c && c.window;
          var u = function (t) {
            this.message = t;
          };
          (u.prototype = new Error()).name = "InvalidCharacterError";
          var l = function (t) {
              throw new u(t);
            },
            s =
              "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
            f = /[\t\n\f\r ]/g,
            d = {
              encode: function (t) {
                (t = String(t)),
                  /[^\0-\xFF]/.test(t) &&
                    l(
                      "The string to be encoded contains characters outside of the Latin1 range.",
                    );
                for (
                  var e,
                    n,
                    r,
                    o,
                    a = t.length % 3,
                    i = "",
                    c = -1,
                    u = t.length - a;
                  ++c < u;

                )
                  (e = t.charCodeAt(c) << 16),
                    (n = t.charCodeAt(++c) << 8),
                    (r = t.charCodeAt(++c)),
                    (i +=
                      s.charAt(((o = e + n + r) >> 18) & 63) +
                      s.charAt((o >> 12) & 63) +
                      s.charAt((o >> 6) & 63) +
                      s.charAt(63 & o));
                return (
                  2 == a
                    ? ((e = t.charCodeAt(c) << 8),
                      (n = t.charCodeAt(++c)),
                      (i +=
                        s.charAt((o = e + n) >> 10) +
                        s.charAt((o >> 4) & 63) +
                        s.charAt((o << 2) & 63) +
                        "="))
                    : 1 == a &&
                      ((o = t.charCodeAt(c)),
                      (i += s.charAt(o >> 2) + s.charAt((o << 4) & 63) + "==")),
                  i
                );
              },
              decode: function (t) {
                var e = (t = String(t).replace(f, "")).length;
                e % 4 == 0 && (e = (t = t.replace(/==?$/, "")).length),
                  (e % 4 == 1 || /[^+a-zA-Z0-9/]/.test(t)) &&
                    l(
                      "Invalid character: the string to be decoded is not correctly encoded.",
                    );
                for (var n, r, o = 0, a = "", i = -1; ++i < e; )
                  (r = s.indexOf(t.charAt(i))),
                    (n = o % 4 ? 64 * n + r : r),
                    o++ % 4 &&
                      (a += String.fromCharCode(255 & (n >> ((-2 * o) & 6))));
                return a;
              },
              version: "0.1.0",
            };
          void 0 ===
            (o = function () {
              return d;
            }.call(e, n, e, t)) || (t.exports = o);
        })();
      }).call(this, n(29)(t), n(30));
    },
    function (t, e) {
      t.exports = (function () {
        "use strict";
        var t = {
            BASE: !0,
            HEAD: !0,
            HTML: !0,
            META: !0,
            NOFRAME: !0,
            NOSCRIPT: !0,
            PARAM: !0,
            SCRIPT: !0,
            STYLE: !0,
            TITLE: !0,
          },
          e = [
            "A",
            "ABBR",
            "ADDRESS",
            "AREA",
            "ARTICLE",
            "ASIDE",
            "AUDIO",
            "B",
            "BASE",
            "BDI",
            "BDO",
            "BLOCKQUOTE",
            "BODY",
            "BR",
            "BUTTON",
            "CANVAS",
            "CAPTION",
            "CENTER",
            "CITE",
            "CODE",
            "COL",
            "COLGROUP",
            "COMMAND",
            "DATALIST",
            "DD",
            "DEL",
            "DETAILS",
            "DFN",
            "DIV",
            "DL",
            "DT",
            "EM",
            "EMBED",
            "FIELDSET",
            "FIGCAPTION",
            "FIGURE",
            "FONT",
            "FOOTER",
            "FORM",
            "H1",
            "H2",
            "H3",
            "H4",
            "H5",
            "H6",
            "HEAD",
            "HEADER",
            "HGROUP",
            "HR",
            "HTML",
            "I",
            "IFRAME",
            "IMG",
            "INPUT",
            "INS",
            "KBD",
            "LABEL",
            "LEGEND",
            "LI",
            "LINK",
            "MAP",
            "MARK",
            "MATH",
            "MENU",
            "META",
            "METER",
            "NAV",
            "NOBR",
            "NOSCRIPT",
            "OBJECT",
            "OL",
            "OPTION",
            "OPTGROUP",
            "OUTPUT",
            "P",
            "PARAM",
            "PRE",
            "PROGRESS",
            "Q",
            "RP",
            "RT",
            "RUBY",
            "S",
            "SAMP",
            "SCRIPT",
            "SECTION",
            "SELECT",
            "SMALL",
            "SOURCE",
            "SPAN",
            "STRONG",
            "STYLE",
            "SUB",
            "SUMMARY",
            "SUP",
            "SVG",
            "TABLE",
            "TBODY",
            "TD",
            "TEXTAREA",
            "TFOOT",
            "TH",
            "THEAD",
            "TIME",
            "TITLE",
            "TR",
            "TRACK",
            "U",
            "UL",
            "VAR",
            "VIDEO",
            "WBR",
          ],
          n = function (t) {
            var e = {},
              n = document.body.appendChild(document.createElement(t)),
              r = window.getComputedStyle(n);
            return (
              [].forEach.call(r, function (t) {
                e[t] = r[t];
              }),
              document.body.removeChild(n),
              e
            );
          };
        return {
          initializeSerializer: function () {
            var r = {};
            return (
              [].forEach.call(e, function (e) {
                t[e] || (r[e] = n(e));
              }),
              function (e) {
                var o,
                  a,
                  i,
                  c,
                  u = [];
                if (e && e.nodeType === Node.ELEMENT_NODE)
                  return (
                    (u = []),
                    (o = e.querySelectorAll("*")),
                    [].forEach.call(o, function (e, o) {
                      var c;
                      t[e.tagName] ||
                        ((a = window.getComputedStyle(e)),
                        (c = (c = e.tagName).toUpperCase()),
                        r[c] || (r[c] = n(c)),
                        (i = r[c]),
                        (u[o] = e.style.cssText),
                        [].forEach.call(a, function (t) {
                          a[t] !== i[t] && (e.style[t] = a[t]);
                        }));
                    }),
                    (c = e.outerHTML),
                    (o = [].map.call(o, function (t, e) {
                      return (t.style.cssText = u[e]), t;
                    })),
                    c
                  );
              }
            );
          },
        };
      })();
    },
    function (t, e, n) {
      "use strict";
      n.d(e, "a", function () {
        return l;
      }),
        n.d(e, "b", function () {
          return s;
        }),
        n.d(e, "e", function () {
          return f;
        }),
        n.d(e, "c", function () {
          return d;
        }),
        n.d(e, "d", function () {
          return p;
        }),
        n.d(e, "f", function () {
          return h;
        });
      var r = n(3),
        o = n(4),
        a = 0,
        i = {
          small: { limit: 10, format: Object(r.format)("") },
          medium: { limit: 1e3, format: Object(r.format)("") },
          large: { limit: null, format: Object(r.format)(".2s") },
        },
        c = {
          small: { limit: 10, format: Object(r.format)(".3f") },
          medium: { limit: 100, format: Object(r.format)(".1f") },
          large: { limit: null, format: Object(r.format)(".2s") },
        },
        u = function (t, e) {
          var n = "large";
          return (
            t < e.small.limit
              ? (n = "small")
              : t < e.medium.limit && (n = "medium"),
            n
          );
        },
        l = function (t) {
          var e = t.length,
            n = 0,
            r = 0,
            a = 0,
            i = 0;
          t.forEach(function (t) {
            (n += t.x), (r += t.y), (a += t.x * t.y), (i += t.x * t.x);
          });
          var c = e * i - n * n,
            u = (r * i - n * a) / c,
            l = (e * a - n * r) / c,
            s = Object(o.min)(t, function (t) {
              return t.x;
            }),
            f = Object(o.max)(t, function (t) {
              return t.x;
            });
          return { x1: s, y1: l * e + u, x2: f, y2: l * f + u };
        },
        s = function (t, e, n) {
          var o = e ? (t / e) * 100 : 0;
          return Object(r.format)(n)(o);
        },
        f = function (t) {
          return t % 1 == 0;
        },
        d = function (t) {
          var e = u(t, c);
          return (0, c[e].format)(t);
        },
        p = function (t) {
          var e = u(t, i);
          return (0, i[e].format)(t);
        },
        h = function (t) {
          var e = ++a;
          return "".concat(t.toString(), "-").concat(e);
        };
    },
    function (t, e) {
      t.exports = c;
    },
    function (t, e) {
      t.exports = u;
    },
    function (t, e, n) {
      "use strict";
      n.r(e),
        n.d(e, "linearGradient", function () {
          return r;
        }),
        n.d(e, "barLoadingMarkup", function () {
          return o;
        }),
        n.d(e, "donutLoadingMarkup", function () {
          return a;
        }),
        n.d(e, "lineLoadingMarkup", function () {
          return i;
        }),
        n.d(e, "stackedAreaLoadingMarkup", function () {
          return c;
        }),
        n.d(e, "sparkLineLoadingMarkup", function () {
          return u;
        }),
        n.d(e, "brushLoadingMarkup", function () {
          return l;
        });
      var r =
          '\n        <defs>\n            <linearGradient id="lgrad" x1="0%" y1="50%" x2="100%" y2="50%" >\n                <stop offset="0" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="33.33%" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="50%" stop-color="#ffffff" stop-opacity="0" />\n                <stop offset="66.66%" stop-color="#ffffff" stop-opacity="0.8" />\n                <stop offset="100%" stop-color="#ffffff" stop-opacity="0.8" />\n            </linearGradient>\n        </defs>\n    ',
        o =
          '\n        <svg class="load-state bar-load-state" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 711 325">\n            '.concat(
            r,
            '\n            <g fill="none" fill-rule="evenodd">\n                <g transform="translate(-15 29)">\n                    <g stroke="#EFF2F5" stroke-dasharray="4 4">\n                        <path d="M.400592911 200.302477L710.674315 200.302477M.400592884 131.172748L710.674315 131.172748M.324410282 64.2071321L710.621499 64.2071321M.291004517.563888874L709.82431.563888889"/>\n                    </g>\n                    <g fill="#D2D6DF" transform="translate(63.08 11)">\n                        <polygon points="-.08 176 23.92 176 23.92 255 -.08 255"/>\n                        <polygon points="50.829 147 74.829 147 74.829 255 50.829 255"/>\n                        <polygon points="254.465 0 278.465 0 278.465 255 254.465 255"/>\n                        <polygon points="458.102 169 482.102 169 482.102 255 458.102 255"/>\n                        <polygon points="152.647 82 176.647 82 176.647 255 152.647 255"/>\n                        <polygon points="356.283 66 380.283 66 380.283 255 356.283 255"/>\n                        <polygon points="559.92 229 583.92 229 583.92 255 559.92 255"/>\n                        <polygon points="101.738 115 125.738 115 125.738 255 101.738 255"/>\n                        <polygon points="305.374 42 329.374 42 329.374 255 305.374 255"/>\n                        <polygon points="509.011 201 533.011 201 533.011 255 509.011 255"/>\n                        <polygon points="203.556 19 227.556 19 227.556 255 203.556 255"/>\n                        <polygon points="407.192 115 431.192 115 431.192 255 407.192 255"/>\n                    </g>\n                </g>\n                <polygon fill="#D2D6DF" fill-rule="nonzero" points="0 295 711 295 711 294 0 294"/>\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          ),
        a =
          '\n        <svg class="load-state donut-load-state" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 376 331">\n            '.concat(
            r,
            '\n            <g fill="none" fill-rule="evenodd">\n                <g transform="translate(116 107)">\n                    <circle cx="72" cy="72" r="72" stroke="#EFF2F5" stroke-linecap="round" stroke-width="9.6"/>\n                    <path stroke="#D2D6DF" stroke-width="19.2" d="M126.153559,119.524055 C137.264629,106.845712 144,90.2321371 144,72.0444604 C144,32.2554036 111.764502,0 72,0"/>\n                    <circle cx="72" cy="72" r="67.2" fill="#FFF"/>\n                </g>\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          ),
        i =
          '\n        <svg\n            class="load-state line-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
            r,
            '\n            <g transform="translate(-10 -10)">\n                <path\n                    id="chart-bg"\n                    class="chart-bg"\n                    style="stroke:#C3C6CF;"\n                    d="M3.4,216.5h707.3 M3.4,160.5h707.3 M3.3,103.5h707.3 M3.3,48.5h707.6 M0.4,276.6H710H0.4z"\n                />\n                <polyline\n                    id="chart-line"\n                    class="chart-line"\n                    style="stroke:#C3C6CF;stroke-width:4;fill:none;"\n                    points="8.8,175.8 62.4,237.7 116.1,184.7 169.7,175.8 223.3,57 277,176.8 330.6,176.8 384.3,122.5 437.9,176.8 491.6,176.8 545.2,218.4 598.8,122.5 652.5,184.7 706.1,135.1 "\n                />\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          ),
        c =
          '\n        <svg\n            class="load-state stacked-area-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
            r,
            '\n            <path\n                id="chart-bg"\n                class="chart-bg"\n                style="stroke:#C3C6CF;"\n                d="M3.4,216.5h707.3 M3.4,160.5h707.3 M3.3,103.5h707.3 M3.3,48.5h707.6 M0.4,276.6H710H0.4z"\n            />\n            <g transform="translate(20 53)">\n                <path\n                    id="chart-area"\n                    strokeLinecap="square"\n                    d="M0.34233103,0.593688165 L709.977885,0.593688189"\n                    transform="translate(.01 227.976)"\n                />\n                <path fill="#C3C6CF" d="M0.528124801,224.014648 L0.528124801,177.734375 L53.3834796,177.734375 C71.5390789,177.734375 86.8277373,168.972754 101.240241,151.662202 C112.578335,138.044258 121.139826,123.110227 136.974507,91.596773 C137.343842,90.8617404 139.300293,86.9654028 139.856735,85.8583549 C155.041692,55.6476711 163.354313,41.0906306 174.319873,27.7179171 C188.951312,9.87459412 204.885845,0.5 223.830634,0.5 C242.123071,0.5 257.291724,8.27027858 270.907992,23.1359289 C281.228683,34.4036118 289.135925,47.1272372 302.542017,72.085092 C303.275893,73.4513345 306.289669,79.0766612 307.063369,80.5168656 C321.41025,107.222876 330.088083,120.97663 341.470704,132.92446 C355.88994,148.05969 371.908861,155.792969 391.654853,155.792969 C412.142049,155.792969 428.763593,152.325614 442.880698,145.765582 C454.197328,140.506893 463.373931,133.679865 473.786035,123.626931 C476.528659,120.978915 486.44777,110.911455 488.791866,108.6483 C502.907223,95.0203436 514.194325,88.9355469 530.135322,88.9355469 C546.532652,88.9355469 559.505909,97.338499 575.973261,115.41103 C579.723508,119.526837 593.103621,135.086814 592.915496,134.871799 C605.09738,148.794859 614.368835,157.635549 625.072091,164.58539 C638.386599,173.230769 652.701021,177.734375 669.279853,177.734375 L673.779853,177.734375 L673.779853,224.014648 L0.528124801,224.014648 Z" />\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          ),
        u =
          '\n        <svg\n            class="load-state sparkline-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
            r,
            '\n            <g transform="translate(0 50)">\n                <path fill="#C3C6CF" d="M0.528124801,224.014648 L0.528124801,177.734375 L53.3834796,177.734375 C71.5390789,177.734375 86.8277373,168.972754 101.240241,151.662202 C112.578335,138.044258 121.139826,123.110227 136.974507,91.596773 C137.343842,90.8617404 139.300293,86.9654028 139.856735,85.8583549 C155.041692,55.6476711 163.354313,41.0906306 174.319873,27.7179171 C188.951312,9.87459412 204.885845,0.5 223.830634,0.5 C242.123071,0.5 257.291724,8.27027858 270.907992,23.1359289 C281.228683,34.4036118 289.135925,47.1272372 302.542017,72.085092 C303.275893,73.4513345 306.289669,79.0766612 307.063369,80.5168656 C321.41025,107.222876 330.088083,120.97663 341.470704,132.92446 C355.88994,148.05969 371.908861,155.792969 391.654853,155.792969 C412.142049,155.792969 428.763593,152.325614 442.880698,145.765582 C454.197328,140.506893 463.373931,133.679865 473.786035,123.626931 C476.528659,120.978915 486.44777,110.911455 488.791866,108.6483 C502.907223,95.0203436 514.194325,88.9355469 530.135322,88.9355469 C546.532652,88.9355469 559.505909,97.338499 575.973261,115.41103 C579.723508,119.526837 593.103621,135.086814 592.915496,134.871799 C605.09738,148.794859 614.368835,157.635549 625.072091,164.58539 C638.386599,173.230769 652.701021,177.734375 669.279853,177.734375 L773.779853,177.734375 L773.77853,204.014648 L0.528124801,204.014648 Z"/>    \n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          ),
        l =
          '\n        <svg\n            class="load-state brush-load-state"\n            xmlns="http://www.w3.org/2000/svg"\n            viewBox="0 0 711 325"\n        >\n            '.concat(
            r,
            '\n            <g transform="translate(0 50)">\n                <path\n                    id="chart-area"\n                    strokeLinecap="square"\n                    d="M0.34233103,0.593688165 L709.977885,0.593688189"\n                    transform="translate(.01 227.976)"\n                />\n                <path fill="#C3C6CF" d="M0.528124801,224.014648 L0.528124801,177.734375 L53.3834796,177.734375 C71.5390789,177.734375 86.8277373,168.972754 101.240241,151.662202 C112.578335,138.044258 121.139826,123.110227 136.974507,91.596773 C137.343842,90.8617404 139.300293,86.9654028 139.856735,85.8583549 C155.041692,55.6476711 163.354313,41.0906306 174.319873,27.7179171 C188.951312,9.87459412 204.885845,0.5 223.830634,0.5 C242.123071,0.5 257.291724,8.27027858 270.907992,23.1359289 C281.228683,34.4036118 289.135925,47.1272372 302.542017,72.085092 C303.275893,73.4513345 306.289669,79.0766612 307.063369,80.5168656 C321.41025,107.222876 330.088083,120.97663 341.470704,132.92446 C355.88994,148.05969 371.908861,155.792969 391.654853,155.792969 C412.142049,155.792969 428.763593,152.325614 442.880698,145.765582 C454.197328,140.506893 463.373931,133.679865 473.786035,123.626931 C476.528659,120.978915 486.44777,110.911455 488.791866,108.6483 C502.907223,95.0203436 514.194325,88.9355469 530.135322,88.9355469 C546.532652,88.9355469 559.505909,97.338499 575.973261,115.41103 C579.723508,119.526837 593.103621,135.086814 592.915496,134.871799 C605.09738,148.794859 614.368835,157.635549 625.072091,164.58539 C638.386599,173.230769 652.701021,177.734375 669.279853,177.734375 L673.779853,177.734375 L673.779853,224.014648 L0.528124801,224.014648 Z" />\n            </g>\n            <rect class="chart-filter" fill="url(#lgrad)" x="-100%" y="0" width="300%" height="100%"></rect>\n        </svg>\n    ',
          );
      e.default = {
        linearGradient: r,
        barLoadingMarkup: o,
        donutLoadingMarkup: a,
        lineLoadingMarkup: i,
        brushLoadingMarkup: l,
        sparkLineLoadingMarkup: u,
        stackedAreaLoadingMarkup: c,
      };
    },
    function (t, e, n) {
      var r = n(12);
      (t.exports = function (t) {
        if (Array.isArray(t)) return r(t);
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    function (t, e) {
      (t.exports = function (t) {
        if (
          ("undefined" != typeof Symbol && null != t[Symbol.iterator]) ||
          null != t["@@iterator"]
        )
          return Array.from(t);
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    function (t, e) {
      (t.exports = function () {
        throw new TypeError(
          "Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.",
        );
      }),
        (t.exports.__esModule = !0),
        (t.exports.default = t.exports);
    },
    ,
    function (t, e) {
      t.exports = function (t) {
        return (
          t.webpackPolyfill ||
            ((t.deprecate = function () {}),
            (t.paths = []),
            t.children || (t.children = []),
            Object.defineProperty(t, "loaded", {
              enumerable: !0,
              get: function () {
                return t.l;
              },
            }),
            Object.defineProperty(t, "id", {
              enumerable: !0,
              get: function () {
                return t.i;
              },
            }),
            (t.webpackPolyfill = 1)),
          t
        );
      };
    },
    function (t, e) {
      var n;
      n = (function () {
        return this;
      })();
      try {
        n = n || new Function("return this")();
      } catch (t) {
        "object" == typeof window && (n = window);
      }
      t.exports = n;
    },
    function (t, e) {
      t.exports = l;
    },
    ,
    ,
    ,
    ,
    ,
    ,
    function (t, e, n) {
      "use strict";
      n.d(e, "c", function () {
        return d;
      }),
        n.d(e, "d", function () {
          return p;
        }),
        n.d(e, "b", function () {
          return h;
        }),
        n.d(e, "a", function () {
          return g;
        });
      var r = n(2);
      function o(t, e) {
        var n =
          ("undefined" != typeof Symbol && t[Symbol.iterator]) ||
          t["@@iterator"];
        if (!n) {
          if (
            Array.isArray(t) ||
            (n = (function (t, e) {
              if (!t) return;
              if ("string" == typeof t) return a(t, e);
              var n = Object.prototype.toString.call(t).slice(8, -1);
              "Object" === n && t.constructor && (n = t.constructor.name);
              if ("Map" === n || "Set" === n) return Array.from(t);
              if (
                "Arguments" === n ||
                /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)
              )
                return a(t, e);
            })(t)) ||
            (e && t && "number" == typeof t.length)
          ) {
            n && (t = n);
            var r = 0,
              o = function () {};
            return {
              s: o,
              n: function () {
                return r >= t.length
                  ? { done: !0 }
                  : { done: !1, value: t[r++] };
              },
              e: function (t) {
                throw t;
              },
              f: o,
            };
          }
          throw new TypeError(
            "Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.",
          );
        }
        var i,
          c = !0,
          u = !1;
        return {
          s: function () {
            n = n.call(t);
          },
          n: function () {
            var t = n.next();
            return (c = t.done), t;
          },
          e: function (t) {
            (u = !0), (i = t);
          },
          f: function () {
            try {
              c || null == n.return || n.return();
            } finally {
              if (u) throw i;
            }
          },
        };
      }
      function a(t, e) {
        (null == e || e > t.length) && (e = t.length);
        for (var n = 0, r = new Array(e); n < e; n++) r[n] = t[n];
        return r;
      }
      var i = 1.2,
        c = 10,
        u = 0.9,
        l = 0.6,
        s = "value",
        f = "label",
        d = function (t, e, n, o) {
          var a,
            d = Object(r.select)(o),
            p = d.text().split(/\s+/).reverse(),
            h = [],
            g = 0,
            m = i * u,
            y = d.attr("y"),
            v = parseFloat(d.attr("dy")),
            b = e * l,
            C = d
              .text(null)
              .append("tspan")
              .attr("x", t)
              .attr("y", y - 5)
              .attr("dy", v + "em")
              .classed(s, !0)
              .style("font-size", e + "px");
          for (
            C.text(p.pop()),
              C = d
                .append("tspan")
                .classed(f, !0)
                .attr("x", t)
                .attr("y", y + c)
                .attr("dy", ++g * m + v + "em")
                .style("font-size", b + "px");
            (a = p.pop());

          )
            h.push(a),
              C.text(h.join(" ")),
              C.node() &&
                C.node().getComputedTextLength() > n - 50 &&
                (h.pop(),
                C.text(h.join(" ")),
                (h = [a]),
                (C = d
                  .append("tspan")
                  .classed(f, !0)
                  .attr("x", t)
                  .attr("y", y + c)
                  .attr("dy", ++g * m + v + "em")
                  .text(a)
                  .style("font-size", b + "px")));
        },
        p = function (t, e) {
          var n =
              arguments.length > 2 && void 0 !== arguments[2]
                ? arguments[2]
                : 0,
            o =
              arguments.length > 3 && void 0 !== arguments[3]
                ? arguments[3]
                : 2;
          t.each(function () {
            var a, i, c, u, l, s, f;
            for (
              a = (t = Object(r.select)(this)).text().split(/\s+/).reverse(),
                c = [],
                u = 0,
                1.2,
                l = t.attr("y"),
                s = parseFloat(t.attr("dy")),
                f = t
                  .text(null)
                  .append("tspan")
                  .attr("x", n)
                  .attr("y", l)
                  .attr("dy", s + "em");
              (i = a.pop());

            )
              if (
                (c.push(i),
                f.text(c.join(" ")),
                f.node() && f.node().getComputedTextLength() > e)
              ) {
                if ((c.pop(), f.text(c.join(" ")), !(u < o - 1))) {
                  c.push("..."), f.text(c.join(" "));
                  break;
                }
                (c = [i]),
                  (f = t
                    .append("tspan")
                    .attr("x", n)
                    .attr("y", l)
                    .attr("dy", 1.2 * ++u + s + "em")
                    .text(i)),
                  t.classed("adjust-upwards", !0);
              }
          });
        },
        h = function (t) {
          var e =
              arguments.length > 1 && void 0 !== arguments[1]
                ? arguments[1]
                : 12,
            n =
              arguments.length > 2 && void 0 !== arguments[2]
                ? arguments[2]
                : "Arial",
            r = document.createElement("canvas"),
            o = r.getContext("2d");
          return (o.font = e + "px " + n), o.measureText(t).width;
        },
        g = function (t, e, n) {
          var r,
            a = [],
            i = 1,
            c = o(t.split(/\s+/).reverse());
          try {
            for (c.s(); !(r = c.n()).done; ) {
              var u = r.value;
              a.push(u),
                h(a.join(" "), e, "Karla, sans-serif") > n &&
                  (a.pop(), (a = [u]), ++i);
            }
          } catch (t) {
            c.e(t);
          } finally {
            c.f();
          }
          return i;
        };
    },
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    ,
    function (t, e, n) {
      "use strict";
      n.r(e),
        n.d(e, "default", function () {
          return t;
        });
      var r = n(8),
        o = n.n(r),
        a = n(9),
        i = n.n(a),
        c = n(23),
        u = n(17),
        l = n(31),
        s = n(6),
        f = n(1),
        d = n(2),
        p = (n(22), n(18)),
        h = n(38),
        g = n(7),
        m = n(21),
        y = n(5),
        v = n(24);
      function b(t, e) {
        var n = Object.keys(t);
        if (Object.getOwnPropertySymbols) {
          var r = Object.getOwnPropertySymbols(t);
          e &&
            (r = r.filter(function (e) {
              return Object.getOwnPropertyDescriptor(t, e).enumerable;
            })),
            n.push.apply(n, r);
        }
        return n;
      }
      function C(t) {
        for (var e = 1; e < arguments.length; e++) {
          var n = null != arguments[e] ? arguments[e] : {};
          e % 2
            ? b(Object(n), !0).forEach(function (e) {
                o()(t, e, n[e]);
              })
            : Object.getOwnPropertyDescriptors
            ? Object.defineProperties(t, Object.getOwnPropertyDescriptors(n))
            : b(Object(n)).forEach(function (e) {
                Object.defineProperty(
                  t,
                  e,
                  Object.getOwnPropertyDescriptor(n, e),
                );
              });
        }
        return t;
      }
      function t() {
        var t,
          e,
          n,
          r,
          o,
          a,
          b,
          x,
          O,
          w,
          A,
          M = { top: 0, right: 0, bottom: 0, left: 0 },
          S = 300,
          E = 300,
          T = !1,
          L = u.easeCubicInOut,
          F = y.e.duration,
          j = 12,
          k = 140,
          D = 45.5,
          R = k + D,
          B = !1,
          P = !1,
          I = !1,
          _ = !0,
          H = !1,
          q = null,
          N = { emptySliceColor: "#EFF2F5", showEmptySlice: !1 },
          G = ".1f",
          U = !0,
          Y = null,
          z = g.default.colorSchemas.britecharts,
          W = function (t) {
            return "".concat(t.percentage, "% ").concat(t.name);
          },
          K = function (t) {
            this._current = t;
          },
          V = function (t) {
            t.outerRadius = k - j;
          },
          Z = function (t, e) {
            return e.quantity - t.quantity;
          },
          X = function (t) {
            return t.quantity;
          },
          Q = function (t) {
            return t.name;
          },
          $ = function (t) {
            var e = t.data;
            return Y[e.name];
          },
          J = Object(c.dispatch)(
            "customMouseOver",
            "customMouseOut",
            "customMouseMove",
            "customClick",
          );
        function tt(c) {
          c.each(function (c) {
            (e = S - M.left - M.right),
              (n = E - M.top - M.bottom),
              (t = (function (t) {
                var e = t.reduce(function (t, e) {
                    return void 0 === e.quantity || null === e.quantity
                      ? t
                      : ((e.quantity = +e.quantity),
                        (e.name = String(e.name)),
                        (e.percentage = e.percentage || null),
                        [].concat(i()(t), [e]));
                  }, []),
                  n = (function (t) {
                    return t.reduce(function (t, e) {
                      return e.quantity + t;
                    }, 0);
                  })(e);
                0 === n && N.showEmptySlice && (P = !0);
                return e.map(function (t) {
                  return (
                    (t.percentage = String(
                      t.percentage || Object(m.b)(t.quantity, n, G),
                    )),
                    t
                  );
                });
              })(c)),
              (function (t) {
                b ||
                  ((b = Object(d.select)(t)
                    .append("svg")
                    .classed("britechart donut-chart", !0)),
                  (function () {
                    var t = b
                      .append("g")
                      .classed("container-group", !0)
                      .attr(
                        "transform",
                        "translate(".concat(S / 2, ", ").concat(E / 2, ")"),
                      );
                    b.append("g").classed("loading-state-group", !0),
                      t.append("g").classed("chart-group", !0),
                      t.append("g").classed("legend-group", !0);
                  })());
                b.attr("viewBox", [0, 0, S, E])
                  .attr(
                    "style",
                    "max-width: 100%; height: auto; height: intrinsic;",
                  )
                  .attr("width", S)
                  .attr("height", E);
              })(this),
              T
                ? b.select(".loading-state-group").html(v.donutLoadingMarkup)
                : (b.select(".loading-state-group svg").remove(),
                  (r = Object(f.pie)().padAngle(0).value(X).sort(Z)),
                  z &&
                    ((A = Object(s.scaleOrdinal)().range(z)),
                    (Y =
                      Y ||
                      A.domain(t.map(Q))
                        .domain()
                        .reduce(function (t, e) {
                          return (t[e] = A(e)), t;
                        }, {}))),
                  (o = Object(f.arc)().innerRadius(D).padRadius(k)),
                  (function () {
                    a && b.selectAll("g.arc").remove();
                    var i = (a = b
                      .select(".chart-group")
                      .selectAll("g.arc")
                      .data(r(t)))
                      .enter()
                      .append("g")
                      .each(K)
                      .each(V)
                      .classed("arc", !0)
                      .append("path");
                    B
                      ? i
                          .merge(a)
                          .attr("fill", $)
                          .on("mouseover", function (t) {
                            rt(this, t, e, n);
                          })
                          .on("mousemove", function (t) {
                            ot(this, t, e, n);
                          })
                          .on("mouseout", function (t) {
                            at(this, t, e, n);
                          })
                          .on("click", function (t) {
                            it(this, t, e, n);
                          })
                          .transition()
                          .ease(L)
                          .duration(F)
                          .attrTween("d", ut)
                      : i
                          .merge(a)
                          .attr("fill", $)
                          .attr("d", o)
                          .on("mouseover", function (t) {
                            rt(this, t, e, n);
                          })
                          .on("mousemove", function (t) {
                            ot(this, t, e, n);
                          })
                          .on("mouseout", function (t) {
                            at(this, t, e, n);
                          })
                          .on("click", function (t) {
                            it(this, t, e, n);
                          });
                    a.exit().remove();
                  })(),
                  b
                    .select(".legend-group")
                    .append("text")
                    .attr("class", "donut-text"),
                  x &&
                    (O = b
                      .selectAll(".chart-group .arc path")
                      .select(nt)
                      .node()) &&
                    (et(O.__data__), ct(O, k, F)),
                  P &&
                    N.showEmptySlice &&
                    (function () {
                      a && b.selectAll("g.arc").remove();
                      (a = b
                        .select(".chart-group")
                        .selectAll("g.arc")
                        .data(r(y.c)))
                        .enter()
                        .append("g")
                        .each(K)
                        .each(V)
                        .classed("arc", !0)
                        .append("path")
                        .merge(a)
                        .attr("fill", N.emptySliceColor)
                        .attr("d", o)
                        .transition()
                        .ease(L)
                        .duration(F)
                        .attrTween("d", ut),
                        a.exit().remove();
                    })());
          });
        }
        function et(t) {
          t.data &&
            U &&
            (b
              .select(".donut-text")
              .text(function () {
                return W(t.data);
              })
              .attr("dy", ".2em")
              .attr("text-anchor", "middle"),
            b.select(".donut-text").call(lt, R));
        }
        function nt(t) {
          if (t.data.id === x) return this;
        }
        function rt(t, e, n, r) {
          et(e),
            J.call("customMouseOver", t, e, Object(d.mouse)(t), [n, r]),
            _ &&
              (q && t !== q && ct(q, k - j, 150),
              O && t !== O && ct(O, k - j),
              ct(t, k));
        }
        function ot(t, e, n, r) {
          J.call("customMouseMove", t, e, Object(d.mouse)(t), [n, r]);
        }
        function at(t, e, n, r) {
          b.select(".donut-text").text(""),
            O && I && !H && (et(O.__data__), ct(O, k)),
            (t !== O || (!I && t === O)) && ct(t, k - j, 150),
            H && (et(t.__data__), ct(t, k), (q = t)),
            J.call("customMouseOut", t, e, Object(d.mouse)(t), [n, r]);
        }
        function it(t, e, n, r) {
          J.call("customClick", t, e, Object(d.mouse)(t), [n, r]);
        }
        function ct(t, e) {
          var n =
            arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : 0;
          Object(d.select)(t)
            .transition()
            .delay(n)
            .attrTween("d", function (t) {
              var n = Object(l.interpolate)(t.outerRadius, e);
              return function (e) {
                return (t.outerRadius = n(e)), o(t);
              };
            });
        }
        function ut(t) {
          var e;
          return (
            (t.innerRadius = 0),
            (e = Object(l.interpolate)({ startAngle: 0, endAngle: 0 }, t)),
            function (t) {
              return o(e(t));
            }
          );
        }
        function lt(t, e) {
          var n = k / 5;
          h.c.call(null, 0, n, e, t.node());
        }
        return (
          (tt.animationDuration = function (t) {
            return arguments.length ? ((F = t), this) : F;
          }),
          (tt.centeredTextFunction = function (t) {
            return arguments.length ? ((W = t), this) : W;
          }),
          (tt.colorMap = function (t) {
            return arguments.length ? ((Y = t), this) : Y;
          }),
          (tt.colorSchema = function (t) {
            return arguments.length ? ((z = t), this) : z;
          }),
          (tt.emptyDataConfig = function (t) {
            return arguments.length ? ((N = t), this) : N;
          }),
          (tt.exportChart = function (t, e) {
            return p.a.call(tt, b, t, e);
          }),
          (tt.externalRadius = function (t) {
            return arguments.length ? ((k = t), this) : k;
          }),
          (tt.hasCenterLegend = function (t) {
            return arguments.length ? ((U = t), this) : U;
          }),
          (tt.hasHoverAnimation = function (t) {
            return arguments.length ? ((_ = t), this) : _;
          }),
          (tt.hasFixedHighlightedSlice = function (t) {
            return arguments.length ? ((I = t), this) : I;
          }),
          (tt.hasLastHoverSliceHighlighted = function (t) {
            return arguments.length ? ((H = t), this) : H;
          }),
          (tt.height = function (t) {
            return arguments.length ? ((E = t), this) : E;
          }),
          (tt.highlightSliceById = function (t) {
            return arguments.length ? ((x = t), this) : x;
          }),
          (tt.internalRadius = function (t) {
            return arguments.length ? ((D = t), this) : D;
          }),
          (tt.isAnimated = function (t) {
            return arguments.length ? ((B = t), this) : B;
          }),
          (tt.isLoading = function (t) {
            return arguments.length ? ((T = t), this) : T;
          }),
          (tt.margin = function (t) {
            return arguments.length ? ((M = C(C({}, M), t)), this) : M;
          }),
          (tt.numberFormat = function (t) {
            return arguments.length ? ((w = t), this) : w;
          }),
          (tt.on = function () {
            var t = J.on.apply(J, arguments);
            return t === J ? tt : t;
          }),
          (tt.orderingFunction = function (t) {
            return arguments.length ? ((Z = t), this) : Z;
          }),
          (tt.percentageFormat = function (t) {
            return arguments.length ? ((G = t), this) : G;
          }),
          (tt.radiusHoverOffset = function (t) {
            return arguments.length ? ((j = t), this) : j;
          }),
          (tt.width = function (t) {
            return arguments.length ? ((S = t), this) : S;
          }),
          tt
        );
      }
    },
  ]).default;
});
//# sourceMappingURL=donut.min.js.map

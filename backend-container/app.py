...
from flask import Flask, url_for, redirect, request, _request_ctx_stack
...
CORS(app, origins='https://0.0.0.0', resources={r"/api/*": {"origins": "https://loveoffootball.io"}})
...
oauth.register(
    ...
    api_base_url=os.getenv('AUTH0_ISSUER_BASE_URL'),
    access_token_url=os.getenv('AUTH0_ISSUER_BASE_URL') + '/oauth/token',
    authorize_url=os.getenv('AUTH0_ISSUER_BASE_URL') + '/authorize',
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        token = get_token_auth_header()  # Ensure this function is defined to extract the token
        try:
            payload = jwt.decode(token, AUTH0_PUBLIC_KEY, algorithms=ALGORITHMS, audience=API_AUDIENCE)
        except jwt.ExpiredSignatureError:
            # Handle expired token, e.g., return an error response
            return "Token expired", 401
        except jwt.JWTClaimsError:
            # Handle invalid claims, e.g., return an error response
            return "Invalid claims", 400
        except Exception as e:
...
AUTH0_DOMAIN = 'dev-uo83nk1cgfqz4tww.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'https://dev-uo83nk1cgfqz4tww.us.auth0.com/api/v2/'

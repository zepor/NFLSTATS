...
function preface(title, options) {
  ...
  let preface = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${md.utils.escapeHtml(
    title,
  )}</title>`;
  ...
  preface +=
    '<link rel="canonical" href="https://spec.openapis.org/oas/latest.html" />';
  ...
  if (options.respec) {
    ...
    preface += fs
      .readFileSync(path.resolve(__dirname, "gist.css"), "utf8")
      .split("\n")
      .join(" ");
    preface += "</style>";
...
for (let l in lines) {
  ...
  if (!inCodeBlock) {
    ...
    if (line.indexOf("xml2rfc.ietf.org") > 0) {
      line = line.replace(
        "https://xml2rfc.ietf.org/public/rfc/html/rfc",
        "https://tools.ietf.org/html/rfc",
      );
      line = line.replace(".html", "");
    }
    line = line.replace(
      /\]\]\(https:\/\/tools.ietf.org\/html\/rfc[0-9]{1,5}\/?(\#.*?)?\)/g,
  ...
  if (!inCodeBlock && line.indexOf("](../") >= 0) {
    ...
    line = line.replace(regExp, function (match, group1) {
      ...
      return (
        "(" +
        url.resolve(
          "https://github.com/OAI/OpenAPI-Specification/tree/master/versions/foo",
          group1,
        ) +
        ")"
      );
  ...
  if (!inCodeBlock && argv.respec && line.startsWith("#")) {
    ...
    if (delta === 0) {
      ...
      prefix =
        "</section>" + "</section>".repeat(Math.abs(delta)) + "<section>";
    }
    prevHeading = heading;
...
s =
  preface(
    `OpenAPI Specification v${argv.subtitle} | Introduction, Definitions, & More`,
    argv,
  ) +
  "\n\n" +
  lines.join("\n");

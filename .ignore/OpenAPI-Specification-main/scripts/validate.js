...
if (process.argv.length < 3) {
  console.log(
    "Usage: validate [--schema=schema] [--version=2021-03-02] [--format=BASIC] path-to-file.yaml",
  );
  console.log(
    "\t--schema: (Default: schema) The name of the yaml schema file to use",
  );
  console.log(
    "\t--version: (Default: 2021-03-02) The version of the yaml schema file to use",
  );
  console.log(
    "\t--format: (Default: BASIC) The JSON Schema output format to use. Options: FLAG, BASIC, DETAILED, VERBOSE",
  );
  process.exit(1);
}
...
(async function () {
  try {
    ...
    fs.readdirSync(`${__dirname}/../schemas/v3.1`, { withFileTypes: true })
      ...
      .map((entry) =>
        fs.readFileSync(`${__dirname}/../schemas/v3.1/${entry.name}`, "utf8"),
      )
    ...
    const schema = await JsonSchema.get(
      `https://spec.openapis.org/oas/3.1/${schemaType}/${schemaVersion}`,
    );
    const validateSchema = await JsonSchema.validate(schema);

    // Validate instance
    const instanceYaml = fs.readFileSync(
      `${process.cwd()}/${process.argv[process.argv.length - 1]}`,
      "utf8",
    );
